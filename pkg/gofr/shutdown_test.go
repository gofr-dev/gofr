...
//
// Which interleaving happens is the scheduler's to decide, so this test is
// opportunistic for every one of its cases, mcp included: it exercises the losing
// ordering only when the ordering falls that way, and none of the three subtests
// is a guarantee on its own. On development the pre-publish ordering does show up
// often enough to fail (measured over five runs: metrics 5/5, http 4/5, mcp 3/5),
// but the proportions are a property of the machine, not of the code — a subtest
// here that passes on development is not evidence that it covers nothing.
//
// The deterministic guard is TestShutdown_BeforeStart_AbandonsTheStart above,
// which fixes the ordering and covers all three servers; this test adds the
// interleavings a fixed ordering cannot reach. Neither replaces the other, and a
// case that looks flaky here should be diagnosed rather than deleted.
