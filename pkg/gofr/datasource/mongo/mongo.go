...
func (c *Client) Connect() {
	...
	if err = m.Ping(context.Background(), nil); err != nil {
		c.logger.Errorf("error connecting mongoDB %s, err:%v", c.config.Host, err)
		return
	}
	...
	c.logger.Logf("connecting to mongoDB at %v to database %v", c.config.Host, c.config.Database)
}
