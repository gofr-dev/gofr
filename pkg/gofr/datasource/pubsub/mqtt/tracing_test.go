...
func TestInjectTraceContext_PreservesExistingAttributes(t *testing.T) {
	...
	existing := map[string]string{"custom-key": "custom-value"}
...
func TestExtractTraceLinks_NilAttributes(t *testing.T) {
...
func TestExtractTraceLinks_NoValidSpanContext(t *testing.T) {
	exporter := tracetest.NewInMemoryExporter()
	tp := sdktrace.NewTracerProvider(sdktrace.WithSyncer(exporter))
	otel.SetTracerProvider(tp)
	otel.SetTextMapPropagator(propagation.TraceContext{})
	...
	attrs := map[string]string{"unrelated-key": "some-value"}
	links := extractTraceLinks(attrs)

	assert.Nil(t, links, "should return nil when no valid span context in attributes")
}
...
func TestStartPublishSpan(t *testing.T) {
	...
	assert.True(t, hasTraceparent, "attrs should contain traceparent")
}
