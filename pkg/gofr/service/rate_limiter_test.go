...
func isAllowed(t *testing.T, err error) bool {
	t.Helper()

	return assert.NoError(t, err)
}

func isRateLimitError(t *testing.T, err error) bool {
	t.Helper()

	var rlErr *RateLimitError

	return assert.ErrorAs(t, err, &rlErr)
}
...
// Ensures constructor wraps *httpService and first request passes.
