...
//
// An attempt made with a context that was already done is the last one: every further attempt would fail
// the same way without reaching the network, and would only be logged and counted as another failed call.
// Its result is returned, which is the result the final retry would have returned.
func (rp *retryProvider) doWithRetry(ctx context.Context, reqFunc func() (*http.Response, error)) (*http.Response, error) {
	...
	for i := 0; i <= rp.maxRetries; i++ {
		...
		if ctxDone || i == rp.maxRetries {
			break
		}

		// This response is replaced by the next attempt; close it so its connection is released.
