...
// TestRunCMD_ExitCode verifies that a CMD app exits non-zero when the handler
// returns an error, and does not exit when the handler succeeds. os.Exit is
// stubbed via the osExit seam so the test binary is not terminated.
func TestRunCMD_ExitCode(t *testing.T) {
	...
	t.Run("handler error exits non-zero", func(t *testing.T) {
		os.Args = []string{"", "fail"}
		...
		app := NewCMD()
		app.SubCommand("fail", func(*Context) (any, error) {
			return nil, errTest
		})

		_ = testutil.StderrOutputForFunc(app.Run)

		assert.Equal(t, 1, exitCode, "expected exit code 1 when the handler returns an error")
	})

	t.Run("handler success does not exit", func(t *testing.T) {
		os.Args = []string{"", "ok"}
		...
		app := NewCMD()
		app.SubCommand("ok", func(*Context) (any, error) {
			return "done", nil
		})

		_ = testutil.StdoutOutputForFunc(app.Run)

		assert.False(t, exitCalled, "expected no exit call when the handler succeeds")
	})
}
