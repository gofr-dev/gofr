...
//	})
//
//	assert.Contains(t, logs, "unable to migrationData transaction")
//}
...
func Test_MigrationRedisGoFrDataUnmarshalFail(t *testing.T) {
	...
	logs := testutil.StdoutOutputForFunc(func() {
		...
		resp := goRedis.NewMapStringStringCmd(context.Background(), map[string]string{"1": string(data)[10:]})

		mocks.Redis.EXPECT().HGetAll(context.Background(), "gofr_migrations").Return(resp)
